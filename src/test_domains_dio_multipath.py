...
def send_to_all_street_lights_multipath(width, height, num_nodes, num_street_lights, tx_range, max_distance, direct_comunication, verbose=False):
    ...
    for street_light_idx in STREET_LIGHT_INDEXES:
        ...
        if verbose:
            ...
            print(f"Projected Routes with Edges Removed Track: {total_hops_projected_routes}")
            print(f"Projected Routes with Disjoint Paths Track: {total_hops_projected_routes2}")
            print(f"Proposed Solution with Edges Removed Domain: {total_hops_domain1}")
            print(f"Proposed Solution with Disjoint Paths Domain: {total_hops_domain3}")
            print(f"Proposed Solution with Common Neighbor Domain: {total_hops_domain2}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_distance', type=int, default=50, help='Distance between street lights')
    parser.add_argument('--direct_comunication', action='store_true', help='Only direct communication. Not range extended')
    parser.add_argument('--width', type=int, default=650, help='Width of the network')
    parser.add_argument('--height', type=int, default=400, help='Height of the network')
