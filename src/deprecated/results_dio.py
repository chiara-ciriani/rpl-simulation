import random
import simpy
from utils.plot_table import print_comparison_table
from utils.plot_results import plot_results

from protocols import rpl_multicast, rpl_operation, rpl_operation_second_approach, rpl_projected_routes
from .network import create_network_with_dio, plot_dodag
from classes.street_light import StreetLight

def send_to_all_street_lights_using_all_protocols(tx_range, num_nodes, num_street_lights, etx, verbose=False):
    env = simpy.Environment()

    # Create the network
    nodes, positions = create_network_with_dio(env, num_nodes, num_street_lights, tx_range, verbose, etx)
...
def run_simulations(num_simulations, tx_range, num_nodes, num_street_lights, etx, verbose):
...
def run_simulations_for_different_node_and_light_counts(num_simulations, tx_range, node_and_light_counts, etx, verbose):
    results = {
        'num_nodes': [],
        'num_street_lights': [],
        'rpl_hops': [],
    ...
    for num_nodes, num_street_lights in node_and_light_counts:
        ...
        rpl_hops, rpl_hops_from_origin, rpl_hops_from_root, rpl_second_approach_hops, rpl_second_approach_hops_from_origin, rpl_second_approach_hops_from_root, projected_routes_hops, proposed_solution_hops = run_simulations(num_simulations, tx_range, num_nodes, num_street_lights, etx, verbose)
...
results = run_simulations_for_different_node_and_light_counts(NUM_SIMULATIONS, TX_RANGE, NODE_AND_LIGHT_COUNTS, ETX, VERBOSE)
